...
When you **merge** a feature branch into the target branch, the app uses the task’s **coding agent** to generate a [Conventional Commits](https://www.conventionalcommits.org/)-style commit message from the diff between the two branches. If the agent is unavailable or returns nothing, a simple fallback message is used.
