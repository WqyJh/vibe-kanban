...
impl Auggie {
    fn build_command_builder(&self) -> Result<CommandBuilder, CommandBuildError> {
        let version = self.variant.as_deref().unwrap_or("0.18.1");
        let mut builder = CommandBuilder::new(format!("npx -y @augmentcode/auggie@{}", version));
...
impl StandardCodingAgentExecutor for Auggie {
    ...
    fn default_mcp_config_path(&self) -> Option<std::path::PathBuf> {
        dirs::home_dir().map(|home| home.join(".auggie").join("settings.json"))
    }
    ...
    fn get_availability_info(&self) -> AvailabilityInfo {
        ...
        let installation_indicator_found = dirs::home_dir()
            .map(|home| home.join(".auggie").join("installation_id").exists())
            .unwrap_or(false);
