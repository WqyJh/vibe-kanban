...
impl Kimi {
    fn build_command_builder(&self) -> Result<CommandBuilder, CommandBuildError> {
        let version = self.variant.as_deref().unwrap_or("1.18.0");
        let mut builder = CommandBuilder::new(format!("npx -y @kimi-dev/kimi@{}", version));
...
impl StandardCodingAgentExecutor for Kimi {
    ...
    fn get_availability_info(&self) -> AvailabilityInfo {
        ...
        let installation_indicator_found = dirs::home_dir()
            .map(|home| home.join(".kimi").join("installation_id").exists())
            .unwrap_or(false);
